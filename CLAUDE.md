...
**Not yet applied to the live database as of this writing** — Claude sessions cannot execute DDL
(no Postgres connection or Management API token, only the PostgREST-backed JS client). The exact
SQL to run in the Supabase SQL editor is in the 2026-07-24 changelog entry below.
...
| **`day_notes` table not yet created:** the day-notes/block-out feature (§9, 2026-07-24) is fully built but the table doesn't exist on the live database yet. | **Blocking** — run the SQL in the 2026-07-24 changelog entry in the Supabase SQL editor. All app code degrades gracefully until then (empty state, no crashes), but the feature is inert. |
