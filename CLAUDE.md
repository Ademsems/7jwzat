...
| 2026-07-22 — Analytics date-range fix | Fixed `getDateRange()` in `app/dashboard/analytics/page.tsx`: the `this-month`/`last-3-months`/`last-6-months` end boundary was `today` instead of the end of the current month, silently excluding bookings dated later in the current month (typically pending/confirmed) from every stat and chart on the page. Since those excluded bookings skew toward not-yet-completed statuses, the page appeared to show completed/cancelled bookings only. No status filter existed anywhere in the query or aggregation logic — verified via direct DB read and an RLS-enforced query as the affected tenant. Fix extends the end boundary to the last day of the current month; `last-month` (already fully in the past) is unchanged. Verified against real data for a test business ("Stitch In Time"): 3 real bookings (2 completed, 1 cancelled) dated July 20/22/24 2026 — old code showed only 2 (dropping the July 24 one since "today" was July 22), fixed code shows all 3. |
